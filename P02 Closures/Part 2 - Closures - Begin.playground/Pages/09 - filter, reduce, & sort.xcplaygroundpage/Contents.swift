...
let shortDwarvesAfterM = arrayOfDwarfArrays.flatMap {
    $0.filter {
        $0 > "M"
    }
}.sorted()
...
getPassStatus(for: ozmaGrades, lowestPass: 70 )

func shortGetPassStatus(for grades: [Int], lowestPass: Int) -> Bool {
    let totalGrade = grades.reduce(0, +)
    return (totalGrade / grades.count) > lowestPass
}
shortGetPassStatus(for: ozmaGrades, lowestPass: 75)
